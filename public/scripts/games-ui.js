// Page UI for games: card overlay navigation, search+filters integration, panel positioning, per-card tag dropdowns
(() => {
  ...
  if (input && grid) {
    ...
    function applySearch() {
      ...
      items.forEach(li => {
        ...
        const match =
          !q || title.includes(q) || tags.includes(q) || type.includes(q) || excerpt.includes(q);
  ...
  function toggleCardTags(trigger, force) {
    ...
    document.querySelectorAll('.game-card.on-top').forEach(c => c.classList.remove('on-top'));
    if (willOpen) {
